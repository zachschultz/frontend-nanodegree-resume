...
var HTMLskillsStart = '<h3 id="skillsH3">Skills at a Glance:</h3><ul id="skills" class="skills-flex-box"></ul>';
...
var HTMLprojectImage = '<img class="lightbox-image" src="%data%">';
...
var HTMLschoolMinor = '<em id="minor"><br>Minor: %data%</em>';
...
function initializeMap() {
  ...
  */
  function locationFinder() {
    ...
    // the locations array
    for (var school in controller.getModel().schools) {
      locations.push(controller.getModel().schools[school].location);
    }

    // iterates through work locations and appends each location to
    // the locations array
    for (var job in controller.getModel().jobs) {
      locations.push(controller.getModel().jobs[job].location);
    }
  ...
  // variable to hold currently open infoWindow
  var activeWindow;
  ...
  */
  function createMapMarker(placeData) {
    ...
    var infoWindow = new google.maps.InfoWindow({
      content: '<div id="hook">'+name+'</div>'
    });

    // hmmmm, I wonder what this is about...
    google.maps.event.addListener(marker, 'click', function() {
      // Check if there is already an infoWindow open
